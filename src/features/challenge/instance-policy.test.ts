...
describe("personal challenge instance creation policy", () => {
  ...
  it("rejects personal instances even when legacy resource fields are supplied", () => {
    expect(createInternalChallengeSchema.safeParse({ ...base, instanceTemplateRef: "web-basic-v1", instanceCpuMilli: "500", instanceMemoryMb: "256", instanceLifetimeMinutes: "60" }).success).toBe(false);
  });
});
