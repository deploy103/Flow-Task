...
export async function createRemoteChallengeInstance(input: { instanceId: string; idempotencyKey: string; templateRef: string; cpuMilli: number; memoryMb: number; lifetimeMinutes: number }) {
  ...
  const response = await fetch(endpoint, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "Idempotency-Key": input.idempotencyKey }, body: JSON.stringify(input), redirect: "error", signal: AbortSignal.timeout(15_000) });
  if (!response.ok) throw new Error(`INSTANCE_PROVIDER_HTTP_${response.status}`);
  return responseSchema.parse(await response.json());
}

export async function stopRemoteChallengeInstance(providerReference: string) {
  const { endpoint, token } = provider();
  const response = await fetch(`${endpoint}/${encodeURIComponent(providerReference)}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` }, redirect: "error", signal: AbortSignal.timeout(15_000) });
