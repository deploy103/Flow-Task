    ...
    <div className="max-w-3xl">
      ...
      <Card className="mt-6">
        <form action={createInternalChallenge} className="space-y-5">
          ...
          <fieldset className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><legend className="px-2 text-sm font-bold">개인 인스턴스 정책</legend><p className="mb-3 text-xs text-slate-500">개인 인스턴스형일 때 운영자가 원격 제공자에 등록한 템플릿 ID와 자원 상한을 입력합니다.</p><div className="grid gap-3 sm:grid-cols-4"><label className="text-sm">템플릿 ID<Input className="mt-2" maxLength={160} name="instanceTemplateRef" placeholder="web-basic-v1" /></label><label className="text-sm">CPU (milli)<Input className="mt-2" max={2000} min={100} name="instanceCpuMilli" type="number" /></label><label className="text-sm">메모리 (MB)<Input className="mt-2" max={2048} min={64} name="instanceMemoryMb" type="number" /></label><label className="text-sm">수명 (분)<Input className="mt-2" max={120} min={5} name="instanceLifetimeMinutes" type="number" /></label></div></fieldset>
