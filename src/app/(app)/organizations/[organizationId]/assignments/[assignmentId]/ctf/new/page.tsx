...
const ERROR_MESSAGES: Record<string, string> = {
  invalid_input: "문제 정보와 플래그를 다시 확인해 주세요.",
...
export default async function NewInternalChallengePage({
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mt-2 text-slate-500">{assignment.title}에 문제 파일과 플래그로 풀 수 있는 정적 CTF를 추가합니다.</p>
      {query.error && ERROR_MESSAGES[query.error] && <p className="mt-5 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700 dark:bg-red-950/50 dark:text-red-200" role="alert">{ERROR_MESSAGES[query.error]}</p>}

      <Card className="mt-6">
        <form action={createInternalChallenge} className="space-y-5">
          <input name="organizationId" type="hidden" value={organizationId} />
          <input name="assignmentId" type="hidden" value={assignmentId} />
          <input name="mode" type="hidden" value={InternalChallengeMode.STATIC_FILE} />
          ...
          <label className="block text-sm font-medium">문제 설명<textarea className="mt-2 min-h-44 w-full rounded-xl border border-slate-200 bg-white p-3 text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50" maxLength={MAX_CHALLENGE_DESCRIPTION_LENGTH} name="description" required /></label>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-medium">분야<select className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50" name="category">{Object.values(ChallengeCategory).map((category) => <option key={category} value={category}>{CHALLENGE_CATEGORY_LABELS[category]}</option>)}</select></label>
          ...
          <label className="block text-sm font-medium">문제 파일 <span className="text-red-500">(필수)</span><Input accept=".pdf,.hwp,.hwpx,.docx,.pptx,.xlsx,.zip,.png,.jpg,.jpeg" className="mt-2" name="resource" required type="file" /><span className="mt-1 block text-xs text-slate-500">비공개 저장 · 최대 {MAX_CHALLENGE_RESOURCE_BYTES / 1024}KB · 실행 파일은 ZIP으로 묶어 등록하세요.</span></label>
          <label className="block text-sm font-medium">힌트 <span className="text-slate-400">(한 줄에 하나)</span><textarea className="mt-2 min-h-28 w-full rounded-xl border border-slate-200 bg-white p-3 text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50" maxLength={(MAX_CHALLENGE_HINT_LENGTH + 2) * MAX_CHALLENGE_HINT_COUNT} name="hints" /></label>
          <fieldset className="space-y-4 rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            ...
            <div className="grid gap-3 sm:grid-cols-2"><label className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800"><input defaultChecked name="caseSensitive" type="checkbox" /> 대소문자 구분</label><label className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800"><input defaultChecked name="trimWhitespace" type="checkbox" /> 앞뒤 공백 제거</label></div>
            ...
            <div className="flex gap-3 rounded-xl bg-indigo-50 p-3 text-sm text-indigo-800 dark:bg-indigo-950/60 dark:text-indigo-200"><ShieldCheck className="shrink-0" size={18} /><p>정답 원문은 저장하지 않고 서버 전용 pepper로 만든 HMAC digest만 저장합니다.</p></div>
          </fieldset>
