...
export default async function CalendarPage({ params, searchParams }: { params: Promise<{ organizationId: string }>; searchParams: Promise<{ month?: string; error?: string; message?: string }> }) {
  ...
  const entries = [
    ...events.map((event) => ({ key: `event-${event.id}`, date: event.startsAt, title: event.title, label: TYPE_LABELS[event.type], href: `/organizations/${organizationId}/calendar/${event.id}/edit` })),
    ...assignments.map((assignment) => ({ key: `assignment-${assignment.id}`, date: assignment.deadline, title: assignment.title, label: "과제 마감", href: `/organizations/${organizationId}/assignments/${assignment.id}` })),
