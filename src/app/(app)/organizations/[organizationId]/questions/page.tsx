import { MentoringRole, QuestionBoardType } from "@prisma/client";
...
export default async function QuestionsPage({ params, searchParams }: { params: Promise<{ organizationId: string }>; searchParams: Promise<{ message?: string }> }) {
  const { organizationId } = await params; const { message } = await searchParams; const { user, membership } = await requireOrganizationAccess(organizationId); const admin = canManageOrganization({ systemRole: user.systemRole, membership }); const mentor = membership?.mentoringRole === MentoringRole.MENTOR;
