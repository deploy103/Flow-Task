...
export default async function QuestionsPage({ params }: { params: Promise<{ organizationId: string }> }) {
  const { organizationId } = await params; const { user, membership } = await requireOrganizationAccess(organizationId); const admin = canManageOrganization({ systemRole: user.systemRole, membership }); const mentor = membership?.role === MembershipRole.MENTOR;
