...
import { sendDepartmentMessage, updateDepartment, updateDepartmentMembers } from "@/features/department/actions";
...
export default async function DepartmentPage({ params, searchParams }: { params: Promise<{ organizationId: string; departmentId: string }>; searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <div className="max-w-4xl"><BackLink href={`/organizations/${organizationId}/departments`} label="부서 목록"/><h1 className="text-3xl font-bold">{department.name}</h1><p className="mt-2 text-slate-500">{department.description ?? "부서 구성원 전용 공간입니다."}</p>
    {(notice.error || notice.message) && <p role="status" className={`mt-5 rounded-xl p-3 text-sm ${notice.error ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>{notice.error ? notice.error === "message_failed" ? "메시지를 보내지 못했습니다. 소속과 입력 내용을 확인해 주세요." : notice.error === "department_update_failed" ? "부서 정보를 저장하지 못했습니다." : "구성원 설정을 저장하지 못했습니다." : notice.message === "members_updated" ? "부서 구성을 저장했습니다." : notice.message === "department_updated" ? "부서 정보를 저장했습니다." : "메시지를 보냈습니다."}</p>}
    {canManage && <Card className="mt-6"><h2 className="text-lg font-bold">부서 정보 관리</h2><form action={updateDepartment} className="mt-4 space-y-3"><input type="hidden" name="organizationId" value={organizationId}/><input type="hidden" name="departmentId" value={departmentId}/><label className="block text-sm font-medium">부서 이름<input name="name" required minLength={MIN_DEPARTMENT_NAME_LENGTH} maxLength={MAX_DEPARTMENT_NAME_LENGTH} defaultValue={department.name} className="mt-2 min-h-11 w-full rounded-xl border px-3 dark:bg-slate-900"/></label><label className="block text-sm font-medium">부서 설명<textarea name="description" maxLength={MAX_DEPARTMENT_DESCRIPTION_LENGTH} defaultValue={department.description ?? ""} rows={3} className="mt-2 w-full rounded-xl border p-3 dark:bg-slate-900"/></label><Button>정보 저장</Button></form></Card>}
