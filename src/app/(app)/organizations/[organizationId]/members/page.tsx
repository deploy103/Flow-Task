import { ClubPosition, MembershipStatus, MentoringRole, SecurityTrack } from "@prisma/client";
...
export default async function MembersPage({
  ...
  const organization = await prisma.organization.findFirst({
    ...
    include: {
      members: {
        ...
        include: { user: true },
        orderBy: [{ position: "asc" }, { securityTrack: "asc" }, { joinedAt: "asc" }],
      },
      invites: {
  ...
  const positionCounts = Object.fromEntries(Object.values(ClubPosition).map((position) => [position, organization.members.filter((member) => member.position === position).length])) as Record<ClubPosition, number>;

  return (
    <div>
      <BackLink href={`/organizations/${organizationId}`} label="조직 홈"/><p className="text-sm font-semibold text-indigo-600">{organization.name}</p><h1 className="mt-1 text-3xl font-bold">구성원 관리</h1><p className="mt-2 text-slate-500">직책·보안 분야·멘토링 역할을 각각 분리해 관리합니다. 초대 코드는 발급 직후 한 번만 표시됩니다.</p>
      {notice.error && <p role="alert" className="mt-5 rounded-xl bg-red-50 p-3 text-sm text-red-700 dark:bg-red-950 dark:text-red-200">{{ role_update_failed: "구성원 분류를 변경할 수 없습니다. 마지막 부장·차장은 운영 직책을 유지해야 합니다.", last_admin: "마지막 조직 관리자는 내보낼 수 없습니다. 다른 구성원에게 관리자 역할을 먼저 부여하세요.", confirmation_mismatch: "구성원 이름이 일치하지 않아 내보내지 않았습니다.", self_removal: "자신은 이 화면에서 내보낼 수 없습니다. 프로필의 동아리 탈퇴를 이용하세요.", remove_failed: "구성원을 내보낼 수 없습니다. 소속 상태를 확인하고 다시 시도하세요.", invitation_revoke_failed: "초대 코드를 폐기할 수 없습니다. 권한과 초대 상태를 확인해 주세요." }[notice.error] ?? "요청을 처리할 수 없습니다."}</p>}
      {notice.message === "role_updated" && <p className="mt-5 rounded-xl bg-green-50 p-3 text-sm text-green-700 dark:bg-green-950 dark:text-green-200">구성원의 직책·분야·멘토링 역할을 변경했습니다.</p>}
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">{Object.values(ClubPosition).map((position) => <Card key={position}><p className="text-sm font-semibold text-indigo-600">{CLUB_POSITION_LABELS[position]}</p><strong className="mt-2 block text-2xl">{positionCounts[position]}명</strong></Card>)}</div>
      <Card className="mt-6 overflow-hidden p-0">
        ...
        <ul className="divide-y divide-slate-200 dark:divide-slate-800">
          {organization.members.map((member) => (
            <li key={member.userId} className="grid gap-4 p-5 lg:grid-cols-[minmax(180px,1fr)_minmax(0,2fr)] lg:items-center">
              <div><p className="font-semibold">{member.user.name}</p><p className="mt-1 text-sm text-slate-500">{member.user.email}{member.user.studentNumber ? ` · ${member.user.studentNumber}` : ""}</p><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 dark:bg-indigo-950 dark:text-indigo-200">{CLUB_POSITION_LABELS[member.position]}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold dark:bg-slate-800">{member.securityTrack ? SECURITY_TRACK_LABELS[member.securityTrack] : "분야 미지정"}</span><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200">{MENTORING_ROLE_LABELS[member.mentoringRole]}</span></div></div>
              <div className="flex flex-col items-stretch gap-2 lg:items-end">
                <form action={updateMemberRole} className="grid w-full gap-2 sm:grid-cols-4">
                  <input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="memberId" value={member.userId} />
                  <select name="position" defaultValue={member.position} aria-label={`${member.user.name} 직책`} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-900">
                    {Object.values(ClubPosition).map((position) => <option key={position} value={position}>{CLUB_POSITION_LABELS[position]}</option>)}
                  </select>
                  <select name="securityTrack" defaultValue={member.securityTrack ?? ""} aria-label={`${member.user.name} 보안 분야`} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-900">
                    <option value="">분야 미지정</option>
                    {Object.values(SecurityTrack).map((track) => <option key={track} value={track}>{SECURITY_TRACK_LABELS[track]}</option>)}
                  </select>
                  <select name="mentoringRole" defaultValue={member.mentoringRole} aria-label={`${member.user.name} 멘토링 역할`} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-900">
                    {Object.values(MentoringRole).map((role) => <option key={role} value={role}>{MENTORING_ROLE_LABELS[role]}</option>)}
                  </select>
