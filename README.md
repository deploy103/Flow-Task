...
- PostgreSQL 기반 이메일 인증 회원가입·로그인, 비밀번호 찾기와 해시 비밀번호·DB 세션
- 사용자 프로필 조회 및 수정
- 조직 생성과 조직 전환
...
Node.js 24와 PostgreSQL이 필요합니다. 인증 메일 발송에는 Resend 계정과 인증된 발신 도메인이 필요합니다.
...
회원가입 이메일 인증과 비밀번호 재설정 메일을 보내려면 `RESEND_API_KEY`와 인증된 발신 주소인 `AUTH_EMAIL_FROM`을 설정합니다. 인증 링크는 `NEXT_PUBLIC_APP_URL`을 기준으로 생성하므로 운영 환경에서는 외부에서 접근 가능한 HTTPS 주소를 사용해야 합니다. API 키와 실제 발신 주소는 저장소에 커밋하지 않습니다. 기존 계정은 마이그레이션 때 인증 완료 상태로 보존되고, 이후 가입한 계정은 24시간 유효한 일회용 링크를 확인하기 전까지 로그인할 수 없습니다. 비밀번호 재설정 링크는 30분 동안 한 번만 사용할 수 있으며 변경 후 기존 세션은 모두 폐기됩니다.
...
- 이메일 인증·비밀번호 재설정 토큰도 SHA-256 해시만 저장하며 일회용·만료 검사를 원자적으로 처리합니다.
